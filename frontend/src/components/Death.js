...
function Death() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8001/api/deaths')
      .then(res => {
  ...
  const handleRowClick = (eventId) => {
    navigate(`/death/${eventId}`);
  };

  if (loading) return <div><h2>Death Events</h2><p>Loading...</p></div>;
  if (error) return <div><h2>Death Events</h2><p>Error: {error}</p></div>;
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <h2 style={{ margin: 0 }}>Death Events</h2>
        </div>
      ...
      {filteredEvents.length === 0 ? (
        <div style={{
          ...
          <p style={{ color: '#6c757d', margin: 0 }}>
            {searchQuery ? 'No death events found matching your search.' : 'No death events found in the database.'}
          </p>
        </div>
      ) : (
