...
class Individual(Base):
    ...
    families_as_spouse = relationship(
        "Family", primaryjoin="Individual.id == Family.spouse1_id"
    )
    families_as_spouse2 = relationship(
        "Family", primaryjoin="Individual.id == Family.spouse2_id"
    )
    families_as_child = relationship("Family", secondary=child_in_family)
    sources = relationship(
        "Source", secondary=individual_source, overlaps="individuals"
    )
...
class Family(Base):
    ...
    spouse1 = relationship(
        "Individual", foreign_keys=[spouse1_id], overlaps="families_as_spouse"
    )
    spouse2 = relationship(
        "Individual", foreign_keys=[spouse2_id], overlaps="families_as_spouse2"
    )
    children = relationship(
        "Individual", secondary=child_in_family, overlaps="families_as_child"
    )
...
class Event(Base):
    ...
    individuals = relationship(
        "Individual", secondary=individual_event, overlaps="events"
    )
...
class Source(Base):
    ...
    individuals = relationship(
        "Individual", secondary=individual_source, overlaps="sources"
    )
...
class Note(Base):
    ...
    individuals = relationship(
        "Individual", secondary=individual_note, overlaps="notes"
    )
...
class Place(Base):
    ...
    )  # Original place name from GEDCOM
