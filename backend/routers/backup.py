...
async def export_backup(db: Session = Depends(get_db)):
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            ...
            zip_file.writestr(
                "data/individuals.json", json.dumps(individuals_data, indent=2)
            )
            ...
            events_data = [
                {
                    ...
                    "event_date": (
                        evt.event_date.isoformat() if evt.event_date else None
                    ),
                    "place": evt.place,
            ...
            for ind in individuals:
                for evt in ind.events:
                    relationships["individual_events"].append(
                        {"individual_id": ind.id, "event_id": evt.id}
                    )

            for fam in families:
                for evt in fam.events:
                    relationships["family_events"].append(
                        {"family_id": fam.id, "event_id": evt.id}
                    )

            for fam in families:
                for child in fam.children:
                    relationships["child_in_family"].append(
                        {"child_id": child.id, "family_id": fam.id}
                    )

            for m in media_list:
                for ind in m.individuals:
                    relationships["media_individuals"].append(
                        {"media_id": m.id, "individual_id": ind.id}
                    )

            for m in media_list:
                for evt in m.events:
                    relationships["media_events"].append(
                        {"media_id": m.id, "event_id": evt.id}
                    )

            zip_file.writestr(
                "data/relationships.json", json.dumps(relationships, indent=2)
            )
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "r") as zip_file:
            ...
            try:
                ...
                print(
                    f"Importing backup from {manifest.get('export_date', 'unknown date')}"
                )
            except KeyError:
                raise HTTPException(
                    status_code=400, detail="Invalid backup file: missing manifest.json"
                )
            ...
            for fam_data in families_json:
                ...
                if fam_data.get("spouse1_id"):
                    fam_data["spouse1_id"] = id_map_individuals.get(
                        fam_data["spouse1_id"]
                    )
                if fam_data.get("spouse2_id"):
                    fam_data["spouse2_id"] = id_map_individuals.get(
                        fam_data["spouse2_id"]
                    )
                fam = Family(**fam_data)
                db.add(fam)
            ...
            for evt_data in events_json:
                ...
                if evt_data.get("event_date"):
                    evt_data["event_date"] = datetime.fromisoformat(
                        evt_data["event_date"]
                    ).date()
                evt = Event(**evt_data)
                db.add(evt)
            ...
            for m_data in media_json:
                ...
                if m_data.get("media_date"):
                    m_data["media_date"] = datetime.fromisoformat(
                        m_data["media_date"]
                    ).date()
                m = Media(**m_data)
                db.add(m)
            ...
            for rel in relationships.get("individual_events", []):
                ...
                if new_ind_id and new_evt_id:
                    db.execute(
                        individual_event.insert().values(
                            individual_id=new_ind_id, event_id=new_evt_id
                        )
                    )
            ...
            for rel in relationships.get("family_events", []):
                ...
                if new_fam_id and new_evt_id:
                    db.execute(
                        family_event.insert().values(
                            family_id=new_fam_id, event_id=new_evt_id
                        )
                    )
            ...
            for rel in relationships.get("child_in_family", []):
                ...
                if new_child_id and new_fam_id:
                    db.execute(
                        child_in_family.insert().values(
                            child_id=new_child_id, family_id=new_fam_id
                        )
                    )
            ...
            for rel in relationships.get("media_individuals", []):
                ...
                if new_media_id and new_ind_id:
                    db.execute(
                        media_individual.insert().values(
                            media_id=new_media_id, individual_id=new_ind_id
                        )
                    )
            ...
            for rel in relationships.get("media_events", []):
                ...
                if new_media_id and new_evt_id:
                    db.execute(
                        media_event.insert().values(
                            media_id=new_media_id, event_id=new_evt_id
                        )
                    )
            ...
            for old_ind_id, old_profile_image_id in profile_image_map.items():
                ...
                if new_ind_id and new_media_id:
                    ind = (
                        db.query(Individual).filter(Individual.id == new_ind_id).first()
                    )
                    if ind:
...
async def export_to_github(config: GitHubBackupRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        repo_response = requests.get(
            f"{github_api}/repos/{config.repo}", headers=headers
        )
        ...
        files_to_commit["data/individuals.json"] = json.dumps(
            individuals_data, indent=2
        )
        ...
        ref_response = requests.get(
            f"{github_api}/repos/{config.repo}/git/ref/heads/{config.branch}",
            headers=headers,
        )
        ...
        if ref_response.status_code == 404:
            ...
            ref_response = requests.get(
                f"{github_api}/repos/{config.repo}/git/ref/heads/{default_branch}",
                headers=headers,
            )
            if ref_response.status_code != 200:
                raise HTTPException(
                    status_code=400, detail="Could not find repository branch"
                )

        current_commit_sha = ref_response.json()["object"]["sha"]

        commit_response = requests.get(
            f"{github_api}/repos/{config.repo}/git/commits/{current_commit_sha}",
            headers=headers,
        )
        ...
        for file_path, content in files_to_commit.items():
            ...
            if blob_response.status_code != 201:
                raise HTTPException(
                    status_code=500, detail=f"Failed to create blob for {file_path}"
                )
        ...
        if tree_response.status_code != 201:
            raise HTTPException(
                status_code=500, detail=f"Failed to create tree: {tree_response.text}"
            )
        ...
        if commit_response.status_code != 201:
            raise HTTPException(
                status_code=500,
                detail=f"Failed to create commit: {commit_response.text}",
            )
        ...
        if ref_update_response.status_code != 200:
            ...
            if ref_create_response.status_code != 201:
                raise HTTPException(
                    status_code=500,
                    detail=f"Failed to update branch: {ref_update_response.text}",
                )
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Error exporting to GitHub: {str(e)}"
        )


@router.post("/github/import")
async def import_from_github(
    config: GitHubBackupRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid backup: missing manifest.json"
            )
        ...
        try:
            ...
            # places.json may not exist in older backups
            print(
                "Note: places.json not found in GitHub backup, skipping places import"
            )
        ...
        for m_data in media_json:
            ...
            try:
                ...
                minio_client.put_object(
                    "media", file_path, io.BytesIO(media_content), len(media_content)
                )
                media_files_imported += 1
            except Exception as e:
                print(f"Warning: Could not import media file {file_path}: {e}")
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Error importing from GitHub: {str(e)}"
        )
